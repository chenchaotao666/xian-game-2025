...
interface AgentSnapshot {
  ...
  teamId?: string;
  hp?: number; // 血量
  maxHp?: number; // 最大血量 (用于显示血条)
  mana?: number;
  position: Position;
  actionDescription?: string; // 例如 "移动至 (3,4)"
  isAlive: boolean;
}
...
interface TurnEvent {
  ...
  rawLog: string;
  parsedAction?: {
    type: string; // "MOVE", "ATTACK", "SKILL", "IDLE"
    from?: Position;
    to?: Position;
    targetId?: string;
    skillName?: string;
  ...
  timestamp?: number; // 用于动画或排序
}

interface TurnData {
  turnNumber: number;
  events: TurnEvent[]; // 按顺序记录本回合所有重要日志事件
  agentStatesAtEndOfTurn: Record<string, AgentSnapshot>; // Keyed by agent ID
  logLines: string[]; // 本回合的原始日志行，用于显示
}

const TILE_SIZE = 40; // px - 增大一点以便显示血条

// --- 日志解析函数 (核心但复杂，这里提供一个非常简化的骨架) ---
